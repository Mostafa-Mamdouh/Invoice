...
namespace AntiXssMiddleware.Middleware
{
    public class AntiXssMiddleware
    {
        ...
        public AntiXssMiddleware(RequestDelegate next, IHostEnvironment env)
        {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            if (!string.IsNullOrWhiteSpace(context.Request.Path.Value))
            {
                ...
                {
                    await RespondWithAnError(context, "XSS in URL").ConfigureAwait(false);
                    return;
                }
            }
            ...
            if (!string.IsNullOrWhiteSpace(context.Request.QueryString.Value))
            {
                ...
                {
                    await RespondWithAnError(context, "XSS in query string").ConfigureAwait(false);
                    return;
                }
            }
            ...
            try
            {
                ...
                {
                    await RespondWithAnError(context, "XSS in request content").ConfigureAwait(false);
                    return;
                }
        ...
        private async Task RespondWithAnError(HttpContext context,string errorMessage)
        {
          ...
          var response= _env.IsDevelopment()
                    ? new ApiException(_statusCode, errorMessage)
                    : new ApiException(_statusCode);
